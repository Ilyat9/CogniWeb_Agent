...
def mock_llm():
    ...
    llm.generate_action = AsyncMock(
        return_value=AgentAction(
            thought="Test thought", tool="navigate", args={"url": "https://example.com"}
        )
    )
    return llm
...
class TestAgentActionValidation:
    ...
    def test_valid_action(self):
        ...
        action = AgentAction(
            thought="Navigate to example", tool="navigate", args={"url": "https://example.com"}
        )
    ...
    def test_invalid_tool_name(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            AgentAction(
                thought="Invalid tool", tool="invalid_tool_name", args={}  # Not in allowed list
            )
    ...
    def test_navigate_requires_url(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            AgentAction(thought="Navigate without URL", tool="navigate", args={})  # Missing 'url'
    ...
    def test_type_text_requires_both_args(self):
        ...
        with pytest.raises(ValidationError):
            AgentAction(thought="Type without text", tool="type_text", args={"element_id": 1})

        # Missing 'element_id'
        with pytest.raises(ValidationError):
            AgentAction(thought="Type without element_id", tool="type_text", args={"text": "hello"})
...
class TestSmartLoopDetection:
    ...
    @pytest.mark.asyncio
    async def test_loop_detected_on_identical_failures(self, mock_settings, mock_browser, mock_llm):
        """Should raise LoopDetectedError on 3 identical failures."""
        orchestrator = AgentOrchestrator(mock_settings, mock_browser, mock_llm)

        # Create identical action
        action = AgentAction(thought="Click button", tool="click_element", args={"element_id": 42})
    ...
    @pytest.mark.asyncio
    async def test_no_loop_on_different_targets(self, mock_settings, mock_browser, mock_llm):
    ...
    @pytest.mark.asyncio
    async def test_no_loop_on_successes(self, mock_settings, mock_browser, mock_llm):
    ...
    @pytest.mark.asyncio
    async def test_loop_detected_on_all_failures(self, mock_settings, mock_browser, mock_llm):
        """Should raise loop if last 5 actions all failed."""
        orchestrator = AgentOrchestrator(mock_settings, mock_browser, mock_llm)

        # Different actions, but all failing
        actions = [
            AgentAction(tool="navigate", args={"url": f"https://example{i}.com"}) for i in range(5)
        ]
...
class TestActionResult:
    ...
    def test_failure_result_with_error(self):
        ...
        result = ActionResult(success=False, message="Action failed", error="ElementNotFound")
