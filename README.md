...
Тот же checkpoint-механизм работает на каждом шаге, не только при капче: агент сохраняет прогресс (`CHECKPOINT_ROTATION_KEEP`, по умолчанию хранятся последние 3 шага) и может продолжить прерванный или упавший прогон с правильного места — `orchestrator.resume(checkpoint_path)` в CLI-сценарии, `POST /task/{task_id}/resume` в API. При повторяющейся одинаковой ошибке инструмента на двух шагах подряд агент автоматически откатывает состояние на пару шагов назад (ограничено `MAX_AUTO_ROLLBACKS`), вместо того чтобы штурмовать одно и то же действие бесконечно. Опционально (`REQUIRE_CONFIRMATION_FOR`, по умолчанию выключено) можно потребовать явного подтверждения перед конкретными инструментами — агент сохранит чекпоинт и остановится, продолжить можно только через `resume(..., confirm=True)`.
