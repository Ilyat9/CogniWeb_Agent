...
Проект следует принципам модульного монолита с четким разделением ответственности:

```
src/
├── config/              # Конфигурация (Pydantic Settings)
├── core/                # Бизнес-логика и модели
├── infrastructure/      # Браузер и LLM сервисы
├── agent/               # ReAct оркестратор
└── utils/               # DOM processing
```
...
# 2. Настроить .env
cp .env.example .env
# Отредактировать OPENAI_API_KEY и MODEL_NAME
...
Агент может выполнять следующие типы задач:
...
**Взаимодействие с UI**:
```
Задача: Найди кнопку "Load More" и кликни её 3 раза, затем сохрани список элементов
```
...
├── comprehensive_completion.py  # Альтернативный entry point
...
│   │   ├── llm.py               # LLMService (OpenAI/OpenRouter API)
│   │   └── __init__.py
│   │
│   ├── agent/
...
**Что агент НЕ умеет**:
- Работать с мобильными приложениями
- Обходить двухфакторную аутентификацию
- Решать CAPTCHA автоматически
- Обрабатывать WebSocket в реальном времени
